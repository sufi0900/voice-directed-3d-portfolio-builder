import { accentOptions, templateOptions } from "@/domain/site-document";
...
export const createVoiceTools = (document: SiteDocument) => [
  ...
  { type: "function", name: "set_color_theme", description: "Change the approved accent or background theme.", parameters: { type: "object", properties: { accent: { type: "string", enum: [...accentOptions] }, background: { type: "string", enum: ["midnight", "ink", "plum", "cloud", "ivory"] } } } },
  { type: "function", name: "set_portfolio_template", description: "Switch the reusable presentation template without changing the user's content.", parameters: { type: "object", properties: { template: { type: "string", enum: [...templateOptions] } }, required: ["template"] } },
