...
export function ProjectDashboard({ projects: initialProjects, email }: { projects: Project[]; email: string }) {
  ...
  async function deleteProject(project:Project){
    if(!window.confirm(`Delete “${project.name}”? Its published page and saved content will be removed. This cannot be undone.`))return;
    setDeleting(project.id);setRenameError("");
    try{await requestJson(`/api/projects/${project.id}`,{method:"DELETE"});setProjects(current=>current.filter(p=>p.id!==project.id));}
    catch(cause){setRenameError(cause instanceof Error?cause.message:"Could not delete. Please retry.");}
    finally{setDeleting("");}
  }
  ...
  return <main className="flow-page wide"><header className="flow-nav"><div className="flow-nav-start"><Link href="/">Home</Link><span>{email}</span></div><Link className="flow-brand" href="/">VOXFOLIO</Link><button className="text-action" onClick={signOut}>Sign out</button></header>
    ...
    <section className="project-grid">{projects.some((entry) => !entry.variant_of_project_id) ? projects.filter((entry) => !entry.variant_of_project_id).map((project) => <article className="project-card" key={project.id}>
      {editingId === project.id ? <div className="project-rename"><label htmlFor={`rename-${project.id}`}>Project name</label><div><input id={`rename-${project.id}`} autoFocus value={draftName} maxLength={80} onChange={(event) => setDraftName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") saveName(project.id); if (event.key === "Escape") cancelRename(); }} /><button type="button" disabled={renaming} onClick={() => saveName(project.id)} aria-label="Save project name"><Check size={17} /></button><button type="button" onClick={cancelRename} aria-label="Cancel rename"><X size={17} /></button></div></div> : <>
        ...
        <div className="project-card-actions"><button type="button" className="danger-action" disabled={deleting===project.id} onClick={()=>void deleteProject(project)}>{deleting===project.id?"Deleting…":"Delete"}</button><Link href={`/studio/${project.id}`}><Settings2 size={15} />Edit Studio</Link><Link href={`/projects/${project.id}/settings`}><Bot size={15} />Visitor Vox settings</Link><button type="button" onClick={() => openVariant(project)}><Sparkles size={15} />Create opportunity page</button>{project.publication && <Link className="live-link" href={`/p/${project.publication.slug}`} target="_blank"><ExternalLink size={15} />View live</Link>}<button type="button" onClick={() => beginRename(project)}><Pencil size={15} />Rename</button></div>
      </>}
    ...
    <section className="project-grid opportunity-list">{projects.filter((entry) => entry.variant_of_project_id).map((project) => <article className="project-card" key={project.id}><div className="project-link"><span>{project.opportunity_status} opportunity page</span><h2>{project.name}</h2><p>Based on {projects.find((item) => item.id === project.variant_of_project_id)?.name ?? "a main portfolio"} · source revision {project.source_revision ?? "—"}</p><small>Draft revision {project.revision}</small>{project.publication ? <div className="project-live-state"><i />Live at <code>/p/{project.publication.slug}</code></div> : <div className="project-draft-state">Review and publish when ready</div>}</div><div className="project-card-actions"><button type="button" className="danger-action" disabled={deleting===project.id} onClick={()=>void deleteProject(project)}>{deleting===project.id?"Deleting…":"Delete"}</button><Link href={`/studio/${project.id}`}><Settings2 size={15} />Review & edit</Link>{project.publication && <Link className="live-link" href={`/p/${project.publication.slug}`} target="_blank"><ExternalLink size={15} />View public page</Link>}</div></article>)}{!projects.some((entry) => entry.variant_of_project_id) && <div className="empty-projects"><h2>No opportunity pages yet</h2><p>Use Create opportunity page on a main portfolio to make a tailored, separately reviewable version.</p></div>}</section>
