...
import { Check, ExternalLink, Pencil, Settings2, Sparkles, X } from "lucide-react";
import { useRouter } from "next/navigation";
import { useState } from "react";
import { createSupabaseBrowserClient } from "@/lib/supabase/browser";

type Project = { id: string; name: string; creation_mode: string; template_id: string | null; revision: number; updated_at: string; variant_of_project_id: string | null; source_revision: number | null; opportunity_status: "canonical" | "draft" | "review" | "published" | "archived"; publication: { slug: string; revision: number; published_at: string } | null };
  ...
  return <main className="flow-page wide"><header className="flow-nav"><div className="flow-nav-start"><Link href="/">Home</Link><span>{email}</span></div><Link className="flow-brand" href="/">VOXFOLIO</Link><button className="text-action" onClick={signOut}>Sign out</button></header>
    ...
    <section className="project-grid">{projects.length ? projects.map((project) => <article className="project-card" key={project.id}>
      {editingId === project.id ? <div className="project-rename"><label htmlFor={`rename-${project.id}`}>Project name</label><div><input id={`rename-${project.id}`} autoFocus value={draftName} maxLength={80} onChange={(event) => setDraftName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") saveName(project.id); if (event.key === "Escape") cancelRename(); }} /><button type="button" disabled={renaming} onClick={() => saveName(project.id)} aria-label="Save project name"><Check size={17} /></button><button type="button" onClick={cancelRename} aria-label="Cancel rename"><X size={17} /></button></div></div> : <>
        <div className="project-link"><span>{project.variant_of_project_id ? `opportunity · ${project.opportunity_status}` : project.creation_mode}</span><h2>{project.name}</h2><p>{project.variant_of_project_id ? `Variant of an approved portfolio · source revision ${project.source_revision ?? "—"}` : project.template_id?.replaceAll("-", " ") ?? "AI-guided foundation"}</p><small>Draft revision {project.revision} · Updated {new Date(project.updated_at).toLocaleDateString()}</small>{project.publication ? <div className="project-live-state"><i />Live at <code>/p/{project.publication.slug}</code><small>Published revision {project.publication.revision}</small></div> : <div className="project-draft-state">Not published yet</div>}</div>
        <div className="project-card-actions"><Link href={`/studio/${project.id}`}><Settings2 size={15} />Edit Studio</Link>{!project.variant_of_project_id && <button type="button" onClick={() => openVariant(project)}><Sparkles size={15} />Create opportunity</button>}{project.publication && <Link className="live-link" href={`/p/${project.publication.slug}`} target="_blank"><ExternalLink size={15} />View live</Link>}<button type="button" onClick={() => beginRename(project)}><Pencil size={15} />Rename</button></div>
      </>}
