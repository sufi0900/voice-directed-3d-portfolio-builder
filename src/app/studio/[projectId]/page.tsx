...
export default async function SavedStudioPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <PortfolioStudio initialDocument={{ ...document, revision: data.revision }} projectName={data.name} persistence="server" initialPublication={live ?? undefined} authenticated userEmail={user.email} />;
}
